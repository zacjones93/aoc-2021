...
// let input = await scrapeSelector(
...
type Direction = [Command, number]

let commands = input.split("\n").map(v => v.split(" ").map(v => parseInt(v) ? parseInt(v) : v))
...
let position = {
  ...
  depth: 0,
  aim: 0,
  addHorizontal: (v:number) => { position.horizontal += v},
  addDepth: (v:number) => {position.depth += (v * position.aim)},
  addAim: (v:number) => {position.aim += v},
  subtractAim: (v:number) => {position.aim -= v}
}
