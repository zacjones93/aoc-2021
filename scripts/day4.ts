...
function buildBoards(data) {
  ...
  let dataAsRows = data.split("\n").slice(1).map((row: string) => {
    let tmpRow = row.split(" ").filter(Boolean)

    if(tmpRow.length > 1) return tmpRow
  }).filter(Boolean)

  let boards: number[][] = dataAsRows.reduce((prev, acc, i, arr)=> {
    ...
    let latestBoard = Object.keys(prev).pop()
    if(prev[latestBoard].length < 5) {
      return {
        ...prev,
        [latestBoard]: [...prev[latestBoard], acc]
      }
    } else {
      return {
        ...prev,
        [parseInt(latestBoard) + 1]: [acc]
      }
    }
  }, {})
...
let {input, boards} = buildBoards(realData)

function checkForRowValue(drawnNumber: string, row: string[]) {
...
function markValuesFound(nDrawn, playingBoards) {
  return Object.keys(playingBoards).map((key) => {
    return playingBoards[key].map((row) => {
      return checkForRowValue(nDrawn, row)
    })
  })
}
...
function checkForWin(board) {
  return board.map((row) => {
    let numberStars = row.reduce((prev, acc) => {
      if(acc.includes("*")) return prev += 1
      return prev
    }, 0)

    if (numberStars === 5) return true 
    return false
  })
}

function checkForRowWin(board) {
  return checkForWin(board).includes(true)
}

function rebuildBoardToCollumnArrays(board) {
    /*
    0,1 1,1 2,1 3,1 4,1
    0,2 1,2 2,2 3,2 4,2
    ...
    00, 10, 20, 30, 40
  */
  return board.map((row, i, arr) => {
    return [arr[0][i], arr[1][i], arr[2][i], arr[3][i], arr[4][i]]
})
}

function checkForColumnWin(board) {
    return checkForWin(rebuildBoardToCollumnArrays(board)).includes(true)
}


function checkForBoardWin(boards) {
  let winningGame = boards.map((b) => {
...
function triggerInput(myInput) {
  ...
  let win = tmpInput.some((numberDrawn: string) => {
    let potentialWinningBoard
     playingBoards = markValuesFound(numberDrawn, playingBoards)

     potentialWinningBoard = checkForBoardWin(playingBoards)
    
     if(potentialWinningBoard) {
      winningNum = numberDrawn
      winningBoard = potentialWinningBoard
      return true
     }
  })
...
function calculateScore({winningBoard, winningNum}) {
  let unstarredSum = winningBoard.map((row) => {
...
visit #1 
  ...
  check fourth row
  check fifth row
check if rows are all marked
check if columns are all marked
