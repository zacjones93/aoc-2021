...
function buildBoards(data) {
  let input2 = data.trim().split("\n")[0];
  let dataAsRows = data.split("\n").slice(1).map((row) => {
    let tmpRow = row.split(" ").filter(Boolean);
    if (tmpRow.length > 1)
      return tmpRow;
  }).filter(Boolean);
  let boards2 = dataAsRows.reduce((prev, acc, i, arr) => {
...
var { input, boards } = buildBoards(realData);
function checkForRowValue(drawnNumber, row) {
